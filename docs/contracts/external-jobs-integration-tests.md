...
### Skip-Semantik (wichtig für Contracts/Validatoren)

In der aktuellen Codebasis wird „übersprungen“ so modelliert:

- Step bleibt **`status: 'completed'`**
- und bekommt `details: { skipped: true, reason: '...' }`

**Harte Regel** (nach Refactor-Pilot 2026-04, siehe `docs/refactor/external-jobs/00-audit.md`):

- Es gibt **keinen** eigenen StepStatus `'skipped'`. Wer ein `'skipped'` schreibt, verletzt den Vertrag.
- Wer ein `'skipped'` liest (Legacy-Daten in alter Mongo-Sammlung), behandelt es semantisch wie `'completed'` mit `details.skipped=true`, aber loggt eine Drift-Warnung. Kein stiller Fallback.

Die `decideTemplateRun`-Reasons sind verbindlich (siehe Char-Tests in `tests/unit/external-jobs/phase-template-*.test.ts`):

- `policy_force` / `policy_skip` / `policy_ignore` — explizite Policy-Steuerung
- `frontmatter_complete_body` — Frontmatter im Body bereits komplett
- `auto_no_gate` — Auto-Mode, kein Gate, kein FM
- `template_completed_fm_ok` / `template_completed_no_repair` / `template_completed_repair` — Callback-Pfad
...
### 5. Policy-Contract (phases vs policies)

Es gibt **zwei Ebenen**, die zusammen die Entscheidung steuern:

- **phases**: aktiviert/deaktiviert die Phase grundsätzlich (`extract/template/ingest`)
- **policies**: bestimmt *wie* entschieden wird (z.B. `force`, `auto`, `ignore`, …)

Wichtig: Die Policy-Namen in älteren Dokumenten (`skipExtractIfArtifactExists`, `forceRecompute`, …) sind nicht zwangsläufig 1:1 die aktuellen Felder im Job-Parameterobjekt.
Wenn du an der Orchestrierung arbeitest, orientiere dich an:

- `src/app/api/pipeline/process/route.ts` (Job-Erstellung + Parameter)
- `src/app/api/external/jobs/[jobId]/start/route.ts` (Entscheidungen: Gates/Directives)
- `src/lib/external-jobs/phase-ingest.ts` (Ingestion: Gate+Policy)
