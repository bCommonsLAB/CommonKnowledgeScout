...
| F11 | (User-Entscheid 2026-06-12, umgesetzt) **Standard-Vorlagen im Sourcecode + Konsistenz-Pflicht**: Pro Inhaltstyp existiert eine aus der `VIEW_TYPE_REGISTRY` generierte Standard-Vorlage (`standard-<typ>`, `lib/templates/default-templates.ts`) — sie erzeugt per Konstruktion genau die Default-Felder (CI-Test verankert das). Leer = „Automatisch": Die Pipeline lädt die Standard-Vorlage des Inhaltstyps (ersetzt den harten `pdfanalyse`-Fallback). Vorlagen-Auswahl ist Experten-Sache (nur unter „Erweitert", mit Live-Konsistenz-Hinweis); das Archiv zeigt die wirksame Vorlage read-only. Beim Speichern wird geprüft: detailViewType-Mismatch **blockiert**, fehlende Pflichtfelder/unbekannte Vorlagen **warnen** (`template-consistency.ts`); ein Inhaltstyp-Wechsel stellt fest gewählte fremde Standard-Vorlagen auf „Automatisch" um. Standard-Namen sind in der Templates-API reserviert (read-only, 409 bei Schreibversuchen) |
