...
| S2 | Einwilligung | MongoDB **neu** `consents`: `(libraryId, userEmail, zweck, erteiltAm, widerrufenAm, bezug?)` — nie im Frontmatter. Tischvereinbarung (Option 1) ist ein Eintrag je Person mit `zweck: tischvereinbarung`, `bezug: tisch`; der Testimonial-Bool `consent` wird nach `consents` migriert (E2) | nur Checkbox | Collection + Route + Widerruf in S11 |
...
`Beobachtungen/{jahr}/{art}`; Dialogformate nutzen den Event-Container des
Bestands (`Veranstaltungen/{reihe}/{runde}/`, die Stimmen darunter).
...
- Archiv (12.09.): „Vorauseilendes Vertrauen - der SHF-Flow im
  Advocatus-Diaboli", „Dialog-Flow neu - Screen-Vorgabe und Testleitfaden",
  „Nachziehliste - was nach dem 12.09. zu aktualisieren ist"
