...
|---|---|---|---|---|
| S0 Einrichten | Sitzung, Tische, Themen (Bezugsobjekte) | Storage: je ein Dokument, `docType: thema` usw., Tisch als flaches Feld `tisch: 2`; Facetten in `doc_meta__<lib>` | Dokumente ja, Zieltyp-Schemas fehlen | Schemas `thema`, `gespraech`, `massnahme` (Klima hat eins), `beobachtung`, `vorhaben` |
...
| S5 Prüfen & Abgeben | Fassung des Erfassers: `metadata`, `markdownBody`, `confidence`, `attribution`, `visibility` | MongoDB `wizard_submissions` (Status `pending`) | ja, ohne `attribution`/`visibility` | C5 + zwei flache Felder |
...
| S8b | Freigegebene Fassung | Storage: Dokument `docType: ergebnis`, `fassung: 3`, `synthese_id: …`; Belege bleiben in MongoDB, im Markdown nur Fußnoten-Marker | nein | Promotion-Variante „Fassung" |
...
ein Konfigurationsfeld kommt an die Library. Das Storage bekommt fünf
Zieltyp-Schemas und drei flache Frontmatter-Felder (`sichtbarkeit`, `anonym`,
`fassung`). Nichts davon verletzt die Flat-Frontmatter-Regel.
...
|---|---|---|---|
| **E0 Regelsatz** | Feld `capture` an der Library (Zieltypen, Zuordnung, Einwilligung, Bewertung, Sichtbarkeit), Settings-Formular, Seed für SHF; Zieltyp-Schemas `thema`, `vorschlag` | — | 3–4 |
...
34 bis 49 PT, also nur zu zweit oder mit dem Schnitt aus der Analyse (E5 auf
