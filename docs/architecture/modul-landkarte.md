...
| A | M4 | `@ks/module-explorer` als **montierbare Wurzelkomponente** (ADR 0008 §4) inkl. Handler-Export + SiteConfig-Gate<br>**Stand**: Gate + API-Namensraum erledigt; Wurzelkomponente offen. Fundament steht: `@ks/ui` (M4b), `@ks/i18n` (M4c), `ClientLibrary` in `@ks/contracts` (M4d), Library-Auswahl in `@ks/shell/react` (M4e) | Voll-App mountet Explorer aus dem Paket |
