...
| A | M4 | `@ks/module-explorer` als **montierbare Wurzelkomponente** (ADR 0008 §4) inkl. Handler-Export + SiteConfig-Gate<br>**Erledigt**: Gate + API-Namensraum (M4), Fundament in M4b–M4e, `ExplorerRoot` unter `@ks/module-explorer/react`. Die Galerie bleibt vorerst in der App und wird als Slot hereingereicht (G3) — sie ist eine eigene Welle. **M4f–M4i erledigt (2026-09-09)**: Die Galerie liegt in `packages/module-explorer/src/gallery/` und kommt aus `@ks/module-explorer/react`; Story, Chat und Website bleiben in der App und kommen als Slots herein (`AGENT-BRIEF-M4f.md`) | **erreicht**: `/explore/[slug]` ist der Montagepunkt (59 Zeilen); der Wurzel-Test montiert ohne Next-Router und ohne Clerk |
