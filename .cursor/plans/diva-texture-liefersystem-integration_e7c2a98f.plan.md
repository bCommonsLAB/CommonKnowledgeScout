...
todos:
  - id: stufe-1-anzeige
    content: "Stufe 1 - Anzeige (PR feature/diva-texture-info-tab): Library-Setting analyzeDivaTextureInfo, Sidecar-Loader, heuristischer Matcher, API-Route, neuer Tab 'DIVA-Info' in file-preview.tsx. Tab zeigt Sidecar-Metadaten + beide Bilder nebeneinander (Basecolor vom Filesystem + Preview vom Sidecar-Image-URL) + Bildwahl-Toggle 'Quellbild fuer Analyse: [basecolor] [supplier-preview]'. Wahl wird im Material-Frontmatter persistiert (analysisSourceImage). Rein deterministisch, kein LLM-Call. Akzeptanz: Flag setzbar, Tab erscheint bei Treffer, beide Bilder sichtbar, Bildwahl persistiert, Unit-Tests fuer Matcher, pnpm lint + pnpm test gruen."
    status: pending
