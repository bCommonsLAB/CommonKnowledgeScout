 ...
 * itemKey = PFTFile (stabiler Textur-Schluessel, konsistent mit Matcher und
 * Bildwahl `analysisSourceImage`).
 *
...
export const DIVA_ATTRIBUTE_KEYS = {
  /** Sidecar GroupName — parallel zu opvGroupName, kein Fallback. */
  stoffgruppe: 'stoffgruppe',
  /** Sidecar OPVGroupName — parallel zu stoffgruppe, kein Fallback. */
  opvGroupName: 'opv_group_name',
  material: 'material',
...
export interface DivaTextureRecord {
  /**
   * Stabiler Item-Key (= PFTFile). Feldname `vcodex` ist historisch;
   * Inhalt ist der PFTFile-Schluessel.
   */
  vcodex: string
  fileName: string
  fileId: string
...
export interface BuildDivaPropsArgs {
  ...
  parentId: string
  /** Dateiname der Sidecar (z.B. optionvalues.json). */
  sourceFile: string
  sourceFileHash?: string
  strategy: string
...
export function buildFlatAttributes(entry: OptionvalueEntry): Record<string, string> {
  ...
  // GroupName und OPVGroupName parallel — kein gegenseitiger Fallback
  // (GroupName kann OPVGroupName bewusst verschleiern).
  const group = entry.GroupName?.trim()
  const opvGroup = entry.OPVGroupName?.trim()
  const material = entry.Material?.trim()
  // Anzeigename (Name) — ergaenzend zum technischen TextureName.
  const name = entry.Name?.trim()
  const rgb = entry.RGB?.trim()
  if (group) attrs[DIVA_ATTRIBUTE_KEYS.stoffgruppe] = group
  if (opvGroup) attrs[DIVA_ATTRIBUTE_KEYS.opvGroupName] = opvGroup
...
/** Schreibt/merged den DIVA-Snapshot (itemKey = PFTFile) in MongoDB. */
export async function writeDivaTextureSnapshot(
  libraryId: string,
  args: BuildDivaPropsArgs,
): Promise<void> {
  const itemKey = resolveTextureItemKey(args.entry)
  await mergeArchiveItemProperties(libraryId, itemKey, buildDivaTextureProperties(args))
}
