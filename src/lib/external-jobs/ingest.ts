 ...
 * MongoDB Vector Search for chat/retrieval functionality. Handles chunking,
...
export async function runIngestion(args: IngestArgs): Promise<IngestResult> {
  const { ctx, savedItemId, fileName, markdown, meta, provider, shadowTwinFolderId, sourceParentId } = args
  const res = await IngestionService.upsertMarkdown(ctx.job.userEmail, ctx.job.libraryId, savedItemId, fileName, markdown, meta || {}, ctx.jobId, provider, shadowTwinFolderId, sourceParentId)
  bufferLog(ctx.jobId, { phase: 'ingest_mongodb_upserted', message: `Upsert abgeschlossen: ${res.chunksUpserted} Chunks` })
  return res
}
