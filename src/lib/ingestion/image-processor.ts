...
export class ImageProcessor {
  ...
  static async processMarkdownImages(
    ...
    // Verarbeite jeden Batch parallel
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      const batch = batches[batchIndex]
      FileLogger.info('ingestion', `Verarbeite Batch ${batchIndex + 1}/${batches.length}`, {
        fileId,
  ...
  static async processSlideImages(
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      const batch = batches[batchIndex]
      FileLogger.info('ingestion', `Verarbeite Slide-Batch ${batchIndex + 1}/${batches.length}`, {
        fileId,
        batchIndex: batchIndex + 1,
        batchSize: batch.length,
      })
