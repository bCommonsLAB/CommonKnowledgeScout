...
export default function LibraryFlowPage() {
  ...
  const [leftPaneParam] = useQueryState("left", parseAsString.withDefault("pdf"))
  ...
  // Sync active library from URL (same intent as LibraryUrlHandler, but Flow is a separate page)
  React.useEffect(() => {
      ...
      try {
        ...
        if (it.type === "file") {
          try {
            ...
            FileLogger.warn("flow-view", "Streaming URL konnte nicht geladen werden", {
              fileId: it.id,
  ...
  // Page title: weniger technisch als "Flow View".
  React.useEffect(() => {
    if (!file?.metadata?.name) return
    document.title = `${file.metadata.name} – Aufbereiten & Publizieren`
  }, [file?.metadata?.name])
    ...
    return (
      <div className="p-4 md:p-6">
        <Alert variant="destructive">
          <AlertTitle>Aufbereiten &amp; Publizieren: Fehler</AlertTitle>
  ...
  return (
    <div className="h-[calc(100vh-4rem)] p-3 md:p-6">
      {/* Expertenansicht: Toolbar + Shadow‑Twin Viewer (kein zusätzlicher Page-Header) */}
