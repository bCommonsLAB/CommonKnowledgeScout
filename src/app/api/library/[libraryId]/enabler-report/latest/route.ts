...
/**
 * GET /api/library/[libraryId]/enabler-report/latest — juengster
 * Enabler-Hebel-Bericht (Stufe 4b, kind 'enabler').
 *
 * Member-only (Owner + Co-Creator), analog overlap-report/latest.
 * 404 = noch kein Lauf.
 */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ libraryId: string }> }) {
  try {
    ...
    const report = await getLatestOverlapReport(libraryId, 'enabler')
