...
/**
 * file-preview/views/video-view.tsx
 *
 * Detail-View fuer Video-Dateien (Video-Player + Tabs Original/Transcript/
 * Transformation/Story/Uebersicht + Pipeline-Sheet).
 *
 * Aus `file-preview.tsx` PreviewContent-Switch (case 'video') ausgegliedert
 * (Welle 3-II-a Phase 2b, Schritt 4b).
 *
 * Struktur ist 1:1 identisch zu `AudioView` — der einzige funktionale
 * Unterschied ist der `leftPaneMode="video"` in `SourceAndTranscriptPane`,
 * sowie die fehlende PDF-Tool-Sichtbarkeit (`isPdf={false}`). Wir halten
 * die Datei dennoch separat, damit zukuenftige Video-spezifische
 * Funktionen (Untertitel, Kapitel-Marker, Frame-Extraktion) hier
 * andocken koennen, ohne `AudioView` zu beruehren.
 *
...
export function VideoView(props: PreviewViewProps) {
  const {
    item,
    provider,
    activeLibraryId,
  ...
  FileLogger.debug('VideoView', 'Video-Pipeline wird gerendert', {
    itemId: item.id,
