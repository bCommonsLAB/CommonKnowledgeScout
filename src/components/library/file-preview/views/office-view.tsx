...
/**
 * file-preview/views/office-view.tsx
 *
 * Detail-View fuer Office-Dateien (docx/xlsx/pptx — gemeinsamer Switch-
 * Case mit drei Trigger-Werten). Zeigt Office-Datei in der Source-Pane
 * + Tabs Original/Transcript/Transformation/Story/Uebersicht +
 * Pipeline-Sheet.
 *
 * Aus `file-preview.tsx` PreviewContent-Switch (case 'docx'|'xlsx'|'pptx')
 * ausgegliedert (Welle 3-II-a Phase 2c, Schritt 4b).
 *
 * Strukturell sehr aehnlich zu `PdfView`. Funktionale Unterschiede zu
 * PdfView:
 * - `kind="office"` ist im PipelineSheet hardcoded (nicht computed).
 * - Story-Tab Beschreibung ist kuerzer (kein "Gallery-Detail Ansicht"-
 *   Hinweis).
 * - Im Transcript-Tab wird `transcript.transcriptItem` direkt
 *   verwendet (nicht `displayTranscriptItem`) — das ist eine bewusste
 *   Bestands-Eigenheit, die wir 1:1 portieren.
 * - `transcriptHeaderExtra` wird im Transcript-Tab NICHT gesetzt
 *   (eine weitere Bestands-Eigenheit).
 * - `isPdf={false}` in TranscriptToolbarActions (kein Page-Splitter).
 *
...
export function OfficeView(props: PreviewViewProps) {
  const {
    item,
    provider,
    activeLibraryId,
      ...
      <Tabs value={infoTab} onValueChange={(v) => setInfoTab(v as typeof infoTab)} className="flex h-full flex-col">
        ...
        <TabsContent value="original" className="min-h-0 flex-1 overflow-hidden p-3">
          <div className="h-full overflow-hidden rounded border">
            {/* Original-Tab nutzt leftPaneMode='pdf' bewusst, da Office-
                Dateien intern in PDF konvertiert und dann als PDF
                angezeigt werden (Bestands-Verhalten, 1:1 portiert). */}
          ...
          {infoTab === 'story' ? (
            <div className="h-full overflow-auto rounded border p-3">
              <div className="mb-2 flex items-center justify-between">
                {/* Office-spezifische Story-Beschreibung — kuerzer als
                    bei Audio/Video/PDF (kein "Gallery-Detail"-Hinweis). */}
                <div className="text-xs text-muted-foreground">
