...
/**
 * file-preview/views/pdf-view.tsx
 *
 * Detail-View fuer PDF-Dateien (PDF-Viewer-Pane + Tabs Original/Transcript/
 * Transformation/Story/Uebersicht + Pipeline-Sheet).
 *
 * Aus `file-preview.tsx` PreviewContent-Switch (case 'pdf') ausgegliedert
 * (Welle 3-II-a Phase 2c, Schritt 4b).
 *
 * Strukturell **fast identisch** zu `VideoView` und `AudioView` — die
 * einzigen funktionalen Unterschiede sind:
 * - `leftPaneMode="pdf"` in `SourceAndTranscriptPane`
 * - `isPdf` (true) in `TranscriptToolbarActions` -> aktiviert die
 *   PDF-Page-Splitting-Tools
 *
...
export function PdfView(props: PreviewViewProps) {
  const {
    item,
    provider,
    activeLibraryId,
