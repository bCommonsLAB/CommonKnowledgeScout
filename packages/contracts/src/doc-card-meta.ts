/**
 * @fileoverview DocCardMeta — die Dokument-Karte als Contract
 *
 * @description
 * `DocCardMeta` ist faktisch der repoweite Dokument-Typ: er beschreibt, was
 * ein Dokument fuer Anzeige, Suche und Auswertung mitbringt. Genutzt wird er
 * auf BEIDEN Seiten — von der Galerie-UI ebenso wie von `vector-repo`,
 * `doc-meta-formatter`, drei `external-jobs`-Phasen und der Website-Navigation.
 *
 * **Warum hier**: Er lag in `src/lib/gallery/types.ts` — einer Datei mit
 * `'use client'`, aus der Server-Code importierte. Solange er dort lag, konnte
 * der Galerie-Ordner nicht ins Modul-Paket wandern, ohne Server-Code
 * mitzureissen oder zu brechen (Galerie-Audit, Befund 1).
 *
 * Nur die Form liegt hier. Der Mapper `mapItemToDocCardMeta` bleibt in der App:
 * er rechnet, und er haengt am `Item`-Modell — das Paket beschreibt, es rechnet
 * nicht (Muster aus `chat-vocabulary.ts`, Welle M4d).
 *
 * @module contracts/doc-card-meta
 */
